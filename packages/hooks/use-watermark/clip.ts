...
export function getClips(
  content: UnwrapRef<NonNullable<WatermarkOptions['content']>> | HTMLImageElement,
  rotate: number,
  ratio: number,
  width: number,
  height: number,
  font: Required<UnwrapRef<NonNullable<WatermarkOptions['font']>>>,
  gapX: number,
  gapY: number,
  space: number,
  ...
  if (content instanceof HTMLImageElement) {
    ...
    ctx.font = `normal ${fontWeight} ${mergedFontSize}px/${height}px ${fontFamily}`;
    ctx.fillStyle = color;
